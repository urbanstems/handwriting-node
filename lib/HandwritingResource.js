...
HandwritingResource.prototype = {
  ...
  _responseBinaryHandler: function (req, callback) {
    var self = this;
    return function (res) {
      var chunks = [];
      ...
      res.on('end', function() {
        ...
        if (chunks.length <= 1 && res.statusCode >= 400) {
          ...
          try {
            ...
            if (res.statusCode === 401) {
              ...
              err = Error.HandwritingError.generate(response.errors);
            }
        ...
        // Binary downloaded - concatenate all chunked buffers
  ...
  _responseHandler: function(req, callback) {
    ...
    // if PNG request
    if (req._headers.accept === 'application/pdf' || req._headers.accept === 'image/png') {
      return this._responseBinaryHandler(req, callback);
    }
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    var headers = {
      ...
      'User-Agent': 'Handwriting/v1 NodeBindings/' + this._handwriting.getConstant('PACKAGE_VERSION')
    };
